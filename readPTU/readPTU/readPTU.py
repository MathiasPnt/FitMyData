...
class PTUmeasurement():
    ...
    def calculate_g2(self, correlation_window, resolution=None,
                     post_selec_ranges=None, channel_start=1, channel_stop=2,
        ...
        if mode == 'symmetric':
            time_vector = np.linspace(-1.*nb_of_bins/2., 1.*nb_of_bins/2., nb_of_bins, dtype='float') * resolution * self.meas.globres
        else:
            time_vector = np.arange(nb_of_bins, dtype='float') * resolution * self.meas.globres
...
if __name__ == '__main__':
    ...
    timetrace_resolution = 0.1    # in seconds
    g2_window = 400*1e-9

    filename = '/Users/mathias/Google Drive/spin7_B18_58pi_HOM_para_000.ptu'
    ...
    with PTUfile(filename) as ptu_file:
        ...
        timetrace_x1, timetrace_y1, timetrace_recnum1 =\
            ptu_meas.timetrace(resolution=timetrace_resolution, channel=1, n_threads=4)
        timetrace_x2, timetrace_y2, timetrace_recnum1 =\
            ptu_meas.timetrace(resolution=timetrace_resolution, channel=2, n_threads=4)

        plt.figure()
        plt.plot(timetrace_x1, timetrace_y1)
        plt.plot(timetrace_x2, timetrace_y2)
        plt.xlabel('Time (s)')
